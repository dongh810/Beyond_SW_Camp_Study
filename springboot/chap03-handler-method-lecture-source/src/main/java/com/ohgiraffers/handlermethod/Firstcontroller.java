...
/* 설명. 현재의 Controller 클래스에 작성할 핸들러 메소드들이 모두 /first/xxx의 요청을 받게 될때 클래스에 어노테이션을 추가할 수 있다. */
...
public class Firstcontroller {
    ...
    /* 설명. 반환형이 void인 핸들러 메소드는 요청 경로 자체가 view의 경로 이름 및 이름을 반환한 것으로 바로 해석이 된다. */
...
//    }
    ...
    @PostMapping("regist")
//   public String registMenu(HttServletRequest request) {
    ...
    @PostMapping("mytest")
    public String testMenu(WebRequest request, Model model) {
        String name = request.getParameter("name");
        System.out.println("name = " + name);
    ...
    * */
     ...
     *  2. name: request parameter의 키 ㄱ밧과 다른 매개변수 명을 사용하고 싶을 때 request parameter의
     *           키 앖을 작성한다.
    * */
    @PostMapping("modify")
    ...
    /* 설명. @ModelAttribute 어노테이션을 활용하면 커맨드 객체를 모델에도 담아주며 어트리뷰트의 키값을 지정할 수 있다.(키값이 없으면 타입의 낙타봉 표기법이 키값이다.) */
